...
- HeroUI v3 also ships `AlertDialog` (`Root`/`Trigger`/`Backdrop`/`Container`/`Dialog`/`Header`/`Heading`/`Body`/`Footer`/`Icon`/`CloseTrigger`) — the destructive-confirm primitive. Composed with `AlertDialog.Trigger`, React Aria supplies the focus trap, `Esc`-cancels, and focus-return-to-trigger, so a destructive confirmation needs no hand-written focus management and never `window.confirm`. `Container` takes `placement` for centred vs. edge placement.
