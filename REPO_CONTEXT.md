...
| `QuotePage/`             | `/cotizar`'s client feature: `QuotePage.tsx` (hydration-gated line list/empty-state/subtotal/`Vaciar lista` confirmation/upgrade-drawer wiring), `QuoteLineRow.tsx` (one line, priced or variant-less), `quote.utils.ts` (`getQuoteTotals`, pure, integer-cents subtotal). |
...
- **`SEARCH_TERM_PATTERN` is too narrow for this catalog's own data, and the failure is silent.** The allowlist (`catalog.constants.ts:39`, `/^[\p{L}\p{N}\s\-_.,&()]+$/u`) omits `"`, `/`, and `°`. Measured against live Strapi on 2026-08-01: **34 of 333 product names (10.2%)** contain a character it rejects (`1/2" Punta Bristol Cromado`, `Broca AAV 135° Split Point`), as do **3,194 of 5,671 variant `diameter` values (56.3%)** (`#10`, `1/2 " / 60° / 3F`). A pattern miss does not raise an error — `tryParseModeValue` (`utils.pagination.ts:50-63`) returns `null` and `getCatalogSelection` calls `redirectToBase()` — so a buyer typing `1/2"` is silently dropped on the unfiltered page-1 catalog. Anything that puts a Strapi-authored name or diameter into `?q=` inherits this. The term reaches Strapi as an Apollo **variable** feeding `containsi`, never interpolated, so the allowlist is input hygiene rather than an injection boundary; note `_` is already allowlisted and is a SQL `LIKE` wildcard under `containsi`.
- The single test asserting the allowlist rejects anything (`__tests__/catalog/_utils.test.ts:362`) uses a forward slash as its unsafe example, so widening the pattern breaks it. Re-point it at a still-rejected character rather than deleting it.
...
| `src/features/QuotePage/QuotePage.tsx`                                                     | `/cotizar` line list: hydration-mounted gate, empty state, subtotal, `Vaciar lista` `AlertDialog` confirmation, wires the drawer's upgrade mode.                                                   |
| `src/features/QuotePage/QuoteLineRow.tsx`                                                  | One quote line, priced or variant-less state; `internalId` never rendered.                                                                                                                         |
| `src/features/QuotePage/quote.utils.ts`                                                    | `getQuoteTotals` — pure, integer-cents subtotal + product/piece counts.                                                                                                                             |
