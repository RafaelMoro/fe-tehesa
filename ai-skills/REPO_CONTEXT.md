...
| `categorias/tornilleria/page.tsx` | Tornillería category route `/categorias/tornilleria`; server component, `generateMetadata` (`index, follow`, canonical `/categorias/tornilleria`), fetches the entire matching set via `fetchAllProductsByCategory("tornilleria")` (no pagination), renders a 3-item `BreadcrumbList` JSON-LD and the `CategoryPage` feature. Ships its own `categorias/tornilleria/error.tsx`/`loading.tsx` — a route nested under `categorias/` is otherwise caught by `categorias/error.tsx`/`loading.tsx`, whose copy is category-index-specific. |
...
| `CategoryPage/`          | `/categorias/tornilleria`'s client feature (`"use client"`): `CategoryPage.tsx` — breadcrumb, hero + shared `WhatsappPanel`, a filtered-count `<p>` (`Intl.NumberFormat("es-MX")`, singular `1 producto`), a filter row (`SearchInput` + `DropdownCategories` reused for subcategories + `DropdownBrands`, all filtering the full product array in memory, AND'd, no navigation/fetch/URL change), `ProductListing` (no `onOpenCatalogSearch` — that action and its helper text are gated off on this page), and `ProductVariantsDrawer` in default mode. Subcategory/brand dropdown options are derived from the loaded set only (unique values present, A→Z via `localeCompare(..., "es")`), not the full taxonomy. |
...
| `src/app/categorias/tornilleria/page.tsx`                                                  | Tornillería category route + `generateMetadata` (`index, follow`), fetches the full set via `fetchAllProductsByCategory`, renders `CategoryPage`; own `error.tsx`/`loading.tsx`.                    |
