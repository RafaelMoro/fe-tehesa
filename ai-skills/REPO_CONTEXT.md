...
| `categorias/tornilleria-fijacion/page.tsx` | Tornillería category route `/categorias/tornilleria-fijacion`; server component, `generateMetadata` (`index, follow`, canonical `/categorias/tornilleria-fijacion`), fetches the entire matching set via `fetchAllProductsByCategory("tornilleria")` (no pagination), renders a 3-item `BreadcrumbList` JSON-LD and the `CategoryPage` feature. Ships its own `categorias/tornilleria-fijacion/error.tsx`/`loading.tsx` — a route nested under `categorias/` is otherwise caught by `categorias/error.tsx`/`loading.tsx`, whose copy is category-index-specific. |
...
| `sitemap.ts`                      | `GET /sitemap.xml` — base pages `/` + `/?page=2..7` + `/categorias` + one static entry per `CATEGORY_PAGE_HREFS` value (currently `/categorias/tornilleria-fijacion`) plus one URL per live category/brand. No `lastModified`/`changeFrequency`/`priority`. Degrades to base pages only if the Strapi taxonomy fetch fails, so a Strapi outage never fails `pnpm build`. |
...
- The other Apollo-backed helpers (`fetchProducts`, `fetchProductsByName`, `fetchProductVariants`, `fetchCategories`, `fetchBrands`, `fetchCategoryProductCounts`) follow the same "throw at the boundary, catch at the edge" contract. The shared contract is documented in a JSDoc block at the top of `src/shared/lib/global.lib.ts`.
...
- `ProductCard` is class-only responsive (no `useMediaQuery()`; that usage and the commented-out `<Image>` code were removed in `4915c10`). Product images: Strapi has **no media field on any content type** (verified 2026-09-14 against `store-tehesa-api` schemas), so the card ships image-less; `ai-research/product-card-redesign.story.md` defines the optional image-prop slot for when the backend adds one.
...
- Product images: no Strapi media field exists yet (see "Styling And UI"); `ProductCard` renders no image today; the redesign story adds an optional image prop that stays unused until the backend field lands. Do not reference localhost Strapi URLs.
...
- Routes nested under `src/app/categorias/` are caught by `src/app/categorias/error.tsx`/`loading.tsx` unless they ship their own — `src/app/categorias/tornilleria-fijacion/` does, with product-specific copy. Add the same pair for any future `src/app/categorias/<slug>/` route.
...
| `src/app/categorias/tornilleria-fijacion/page.tsx`                                                  | Tornillería category route + `generateMetadata` (`index, follow`), fetches the full set via `fetchAllProductsByCategory`, renders `CategoryPage`; own `error.tsx`/`loading.tsx`.                    |
...
| `src/app/sitemap.ts`                                                                       | `/sitemap.xml` — base pages (incl. `/categorias` + `CATEGORY_PAGE_HREFS`) + live category/brand URLs; degrades to base pages if Strapi is unreachable.                                              |
...
| `src/components/ProductCard.tsx`                                                           | Product card UI (plain `<article>`, not HeroUI `Card`); two-CTA footer (`Explorar…` + outline secondary `Agregar y elegir después`) or, when `variantCount === 1`, a single `Agregar 1 pieza` CTA with pending/failure states. Optional `image?: { src; alt }` prop, unused by any caller yet. |
