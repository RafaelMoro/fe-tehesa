...
| `sitemap.ts`                      | `GET /sitemap.xml` — base pages `/` + `/?page=2..7` + `/categorias` plus one URL per live category/brand. No `lastModified`/`changeFrequency`/`priority`. Degrades to base pages only if the Strapi taxonomy fetch fails, so a Strapi outage never fails `pnpm build`. |
...
| `CategoriesPage/`        | `/categorias`'s server feature (no client directive): `CategoriesPage.tsx` (breadcrumb, hero + env-gated WhatsApp panel, `N categorías` counter, category grid or empty state — receives the already-sorted `CategoryWithCount[]` as a prop), `CategoryCard.tsx` (`<article>` with initial-letter badge, `N productos` pill via `Intl.NumberFormat("es-MX")` — hidden when `productCount` is `null` — `<h2>` name, and a `Ver categoría` CTA rendered `aria-disabled` until per-category pages exist). |
...
| `ui/organisms` | `Header` (sticky, every route): a dark utility bar (tagline + WhatsApp "Solicitar cotización"/"Cotizar" link, hidden when `NEXT_PUBLIC_WHATSAPP_NUMBER` is unset), wordmark, desktop nav (`Productos` + `Categorías`/`Marcas` HeroUI `Dropdown`s listing live taxonomy as **disabled** items — no dedicated category/brand pages exist yet — hidden when taxonomy is empty), and a mobile row (lupa, `CartCount`, `MobileMenu`). The lupa only renders on `/` and dispatches `CATALOG_SEARCH_OPEN_EVENT` on `window`, which `Home.tsx` listens for to open its existing `CatalogSearchDrawer` — no second search UI. `MobileMenu` is a right-side HeroUI `Drawer` with a `Productos` link, `Categorías`/`Marcas` `Accordion` sections (disabled rows, one open at a time), and a footer with the WhatsApp CTA plus `ToggleDarkMode showLabel`. **Gotcha:** react-aria-components' `MenuItem` strips unrecognized `aria-*` props (e.g. `aria-current`) even though `data-*` passes through — the active-taxonomy-item indicator in `Header.tsx`'s dropdown uses a visually-hidden `" (actual)"` text suffix instead; the plain `<span>` rows inside `MobileMenu`'s accordions are unaffected and do carry real `aria-current`. Since the categories-page story, the `Categorías` dropdown/accordion end with a real `Ver todas las categorías` row linking to `/categorias` (hidden on `/categorias` itself), and the `Categorías` trigger/accordion header carry the same active underline/highlight + `" (actual)"` treatment on `/categorias` that `Productos` carries on `/`. |
