...
| `QuotePage/`             | `/cotizar`'s client feature: `QuotePage.tsx` (hydration-gated line list/empty-state/subtotal/`Vaciar lista` confirmation/upgrade-drawer wiring, plus Story 3's checking/failed banners and stepper-unmount focus handling), `QuoteLineRow.tsx` (five line states — priced incl. changed-price, variant-less, variant-gone, product-gone, no-price), `quote.utils.ts` (`getQuoteTotals` with optional `LineChecks`, `buildProductSearchHref`), `useQuoteRevalidation.ts` (batches one `/api/catalog/revalidate` call per mount, exposes `pageStatus`/`checks`/`retry`). |
