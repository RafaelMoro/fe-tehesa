...
| `src/shared/constants/catalog.constants.ts` | **New:** `CAT_VAL_007` / `MSG_CAT_VAL_007`, `REVALIDATE_MAX_IDS` |
...
Status: pending
Explanation: The comps give both `precio guardado`. They are different facts — "we tried and could not" versus "we have not tried" — but the second is only visible for the few hundred milliseconds before the request resolves, and after `Reintentar` fails they are the same fact for the buyer. Recommendation: **do not distinguish them.** Raising it because it is the kind of thing that gets "fixed" later by someone who notices the collapse and assumes it was an oversight.

III: Question: Is `Buscar alternativa` a link or a button?
Status: answered — **a `next/link` anchor** to `/?mode=name&q=<encoded product name>` (user decision, 2026-08-01).
...
- **`Buscar alternativa` routes Strapi-authored text through a validated param.** `SEARCH_TERM_PATTERN` (`catalog.constants.ts:39`) is an allowlist; a product name outside it produces a validation error page rather than a search. The name comes from an editor, not from us.
