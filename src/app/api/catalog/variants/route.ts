...
export async function GET(request: Request) {
  ...
  if (!documentId.ok) return failure(documentId.error.code, documentId.error.message)
  if (!variantPageSize.ok) return failure(variantPageSize.error.code, variantPageSize.error.message)

  try {
    const variants = await fetchProductVariants({ documentId: documentId.value })
