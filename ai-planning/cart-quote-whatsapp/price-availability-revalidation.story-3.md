...
**Implementation note (Phase 3 vs Phase 4 boundary):** this section's bullet list literally says to pass `check={checks[key]}` and `pageStatus` to each `QuoteLineRow`, and to add the stepper-unmount focus effect, in this phase. Deferred both to Phase 4 instead: `QuoteLineRowProps` does not accept those props until Phase 4 modifies `QuoteLineRow.tsx`, so passing them now would fail `tsc --noEmit`. The focus-on-unmount effect is also meaningless before Phase 4, since no row can lose its stepper until the five-state rendering exists — wiring it early would make focus jump to the list region whenever a check merely resolves, with nothing having disappeared, which is not the described behavior. Everything else in this phase (hook, totals, banner) is unaffected and delivered as planned.
...
| --- | --- | --- | --- | --- |
| AC1 - single round trip, batched ids, no request on empty list | Phase 1, Phase 3 | `GET /api/catalog/revalidate?variantIds=<v1>,<v2>&productIds=<p1>` 200 with both arrays in one envelope; 11+ ids all returned; dev log shows one `/api/catalog/revalidate` line per `/cotizar` load with a seeded cart and none on stepper press | Validated | Route half validated in Phase 1. Client half validated by `revalidation.test.tsx`: empty cart issues zero requests; a seeded cart issues exactly one; a stepper press after resolution does not refire; ids deduped and product ids include variant lines' products. The literal Network-tab observation is still M12 |
| AC2 - changed price: struck previous, `TOTAL ACTUAL`, subtotal on current, cents comparison | Phase 3, Phase 4 | none — rendered client-side after the fetch | Cannot validate | Fully covered by `revalidation.test.tsx` (struck previous price + sr-only "Precio anterior", `Total actual` header, subtotal on current price, and the `648.9` vs `648.90` cents-equal case renders no change). Dev-server check n/a (client-rendered); live-Strapi confirmation is M4 |
| AC3 - variant-gone vs product-gone states, both excluded, neither auto-removed | Phase 3, Phase 4 | none — rendered client-side after the fetch | Cannot validate | Fully covered by `revalidation.test.tsx`: variant-gone (copy, no stepper, excluded from subtotal, `Elegir otra medida` opens the drawer), product-gone (copy, `Buscar alternativa` link, precedence over the variant-less "Sin variante seleccionada" row), neither line auto-removed. Dev-server check n/a (client-rendered); live-Strapi confirmation is M5, M6, M11 |
| AC4 - failure never blocks: banner + `Reintentar`, snapshot prices, `precio sin confirmar`, controls work | Phase 1, Phase 3, Phase 4 | With `STRAPI_HOST` unset: `GET /api/catalog/revalidate?variantIds=<id>` 400 `CAT_ENV_001` **and** `GET /cotizar` still 200 | Cannot validate | Phase 1 half confirmed (`CAT_ENV_001` with env unset). Full client behavior (banner, snapshot price, working controls, `Reintentar`, and now the `precio sin confirmar` affix) covered by `revalidation.test.tsx`. Page-level dev-server check is n/a (client-rendered) — live-Strapi confirmation is M3 |
| AC5 - ephemeral state only; no persisted field, no schema bump, no migrate/rehydrate change | Phase 3 | none — no runtime surface | Validated | `git diff --stat` for Phase 3 confirms `cart.store.ts` and `cart.constants.ts` are untouched; checks live only in `useQuoteRevalidation`'s React state. `pnpm exec tsc --noEmit` clean |
...
| AC7 - variant with `pricing: null` → fifth state, stepper kept, only `Quitar` | Phase 3, Phase 4 | none — rendered client-side after the fetch | Cannot validate | Fully covered by `revalidation.test.tsx`: copy, stepper kept and functional, only `Quitar` offered (no `Elegir otra medida`/`Buscar alternativa`), excluded from subtotal, no price block. Dev-server check n/a (client-rendered); live-Strapi confirmation is M7 |
| AC8 - `Buscar alternativa` reaches a filtered search; widened allowlist | Phase 2, Phase 4 | `GET /api/catalog/search?q=1%2F2%22%20Punta%20Bristol%20Cromado` 200; `/?mode=name&q=1%2F2%22&page=1` 200 with no redirect; `/?mode=name&q=Punta%20Bristol%20Cromado&page=1` 200 containing the product; `q=%3Cscript%3E` still 400 `CAT_VAL_006` | Validated | Route half confirmed against live Strapi data in Phase 2. Consumer half confirmed by `revalidation.test.tsx`: the rendered `Buscar alternativa` link's `href` matches `buildProductSearchHref` exactly, and no link renders when the name strips to nothing. The click itself is still M9 |
