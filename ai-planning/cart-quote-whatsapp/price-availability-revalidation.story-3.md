...
| AC4 - failure never blocks: banner + `Reintentar`, snapshot prices, `precio sin confirmar`, controls work | Phase 1, Phase 3, Phase 4 | With `STRAPI_HOST` unset: `GET /api/catalog/revalidate?variantIds=<id>` 400 `CAT_ENV_001` **and** `GET /cotizar` still 200 | Cannot validate | Phase 1 half confirmed (`CAT_ENV_001` with env unset). Phase 3 half confirmed by `revalidation.test.tsx`: failure banner (`role="alert"`, both UI II lines verbatim), snapshot price still shown, quantity/`Quitar` still work, `Reintentar` refires and clears the alert on success. The `precio sin confirmar` affix is Phase 4 (row rendering); page-level dev-server check is `Cannot validate` (client-rendered) — covered by tests + M3 |
