...
| AC5 — Entry points + SEO + tests | 2, 3 | `curl /categorias` has exactly one `href="/categorias/tornilleria"`; `/categorias/tornilleria` body has `Categorías…(actual)`; `/sitemap.xml` lists `/categorias/tornilleria`; `<title>`/canonical/robots on the route | Not validated | SEO literals (title/canonical/robots) validated this phase via `tornilleria-metadata.test.ts` + curl; entry-point links, active state, and sitemap are Phase 3 work |
