...
| --- | --- | --- | --- | --- |
| AC1 — Route + data (fetch-all by `pageInfo`, `subcategory` selected, no pagination, error propagation, empty state) | 1, 2 | `GET /categorias/tornilleria` 200; `grep -o "<article" \| wc -l` = 107; no `Página anterior`/`Siguiente`; kicker labels present | Validated | Paging loop + empty set proved by `global.lib.test.ts`; error propagation to the route's own `error.tsx` covered by `tornilleria-error.test.tsx` — bad-token manual check still pending (dev-server validation only, not yet run) |
| AC2 — Page structure (breadcrumb, hero, panel, counter) | 2 | Body contains `aria-label="Ruta"`, `aria-current="page"`, `<h1>…Tornillería y fijación industrial`, `Categoría`, `Cotizar ahora`, `107 productos` | Validated | Filtered/singular counter covered by `CategoryPage.test.tsx`; 2-column layout at `lg` is manual (pending) |
| AC3 — Frontend filters | 2 | Body contains `Buscar tornillos, tuercas, pernos...`, `Filtrar subcategorías`, `Filtrar marcas`; idle body lacks `Limpiar filtros` and `Buscar en todo el catálogo` | Validated | Stacking/clear/zero-match verified by `CategoryPage.test.tsx`; manual click-through (Network tab, no URL change) still pending |
| AC4 — Card + drawer | 1, 2 | `grep -o "Tornillería / [^<]*" \| sort \| uniq -c` shows 10 labels on `/categorias/tornilleria` (counts sum to 107); `grep -c "Tornillería / "` = 0 on `/?page=1` | Validated | Drawer open covered by `CategoryPage.test.tsx`; add-to-cart flows covered by `ProductCard.test.tsx`; manual click-through still pending |
| AC5 — Entry points + SEO + tests | 2, 3 | `curl /categorias` has exactly one `href="/categorias/tornilleria"` and 15 `aria-disabled="true"` CTAs; `/categorias/tornilleria` body has `Categorías…(actual)`; `/sitemap.xml` lists `/categorias/tornilleria` once; `<title>`/canonical/robots on the route | Validated | All dev-server checks pass exactly as specified; `Header.test.tsx`/`CategoriesPage.test.tsx`/`sitemap.test.ts` cover dropdown/drawer link rows (not in SSR HTML); `pnpm lint`, `pnpm exec tsc --noEmit`, `pnpm test` (418 passed, 1 pre-existing skip), `pnpm build` all green |
