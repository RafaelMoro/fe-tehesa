...
| AC6 – SEO | Phase 2 (metadata, JSON-LD), Phase 3 (sitemap) | `GET /marcas` `<title>` = `BRANDS_TITLE`, canonical `/marcas`, robots `index, follow`, `"@type":"BreadcrumbList"`; `GET /sitemap.xml` contains `/marcas` with Strapi up and down | Not validated | Metadata/JSON-LD validated this phase; sitemap entry is Phase 3 scope. |
