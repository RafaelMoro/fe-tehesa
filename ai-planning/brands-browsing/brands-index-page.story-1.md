...
| AC2 – Active state | Phase 1 (code), Phase 2 (route exists) | `GET /marcas` 200 contains `Marcas` trigger with ` (actual)`, `Categorías` trigger without, `Productos` without `aria-current`; `GET /categorias` has ` (actual)` only on `Categorías` | Not validated | Underline/tint are styling → manual. Mobile accordion trigger `(actual)`: Header test. |
| AC3 – Page structure | Phase 2 | `GET /marcas` 200; `grep -c "<article"` = 6; contains `aria-label="Ruta"`, `aria-current="page">Marcas`, `Explora el catálogo por marca`, `Seis marcas en almacén`, `6 marcas en almacén`, `Ordenadas por fondo de catálogo`, one `<main`; no `Marca Libre`; `STRAPI_HOST` unreachable → root `error.tsx` | Not validated | Empty state (0 brands) has no live fixture → `BrandsPage.test.tsx`. |
| AC4 – Brand card | Phase 2 | `GET /marcas`: `<h2>` order `WESTON … CLEVELAND`, no `Clevaland`; `En almacén` × 6; `Ver productos` × 6; `href="#"` × 0; no `productos</span>` pill | Not validated | Tags as `<li>`, CTA not focusable, enabled-CTA branch (`BRAND_PAGE_HREFS` non-empty) → `BrandsPage.test.tsx` covers only the disabled branch this story; the link branch is exercised in Story 2. |
| AC5 – Closing panel + note | Phase 2 | `GET /marcas` contains `href="/categorias/tornilleria-fijacion"`, `¿No ves tu marca?`, `Buscar por categoría` → `href="/categorias"`, `Cotizar por WhatsApp` with `wa.me` + `target="_blank"` + `rel="noopener noreferrer"`; `NEXT_PUBLIC_WHATSAPP_NUMBER=` → button absent, 200 | Not validated | Exact prefill text vs `WHATSAPP_BRANDS_MESSAGE` → `BrandsPage.test.tsx`. |
| AC6 – SEO | Phase 2 (metadata, JSON-LD), Phase 3 (sitemap) | `GET /marcas` `<title>` = `BRANDS_TITLE`, canonical `/marcas`, robots `index, follow`, `"@type":"BreadcrumbList"`; `GET /sitemap.xml` contains `/marcas` with Strapi up and down | Not validated | |
| AC7 – Tests | Phases 1–3 | `pnpm test` green (not a dev-server check) | Cannot validate | Proven by the test run: `Header.test.tsx`, `brands/BrandsPage.test.tsx`, `seo/brands-metadata.test.ts`, `seo/sitemap.test.ts`. |
