...
| AC2 — 11 new pages carry table copy, live counts, empty/single sets render | 2 | per-slug title/description/canonical/H1 grep (all 11 match table verbatim); intro grep on `carburo` (1) + `tornilleria-fijacion` (1) confirms intro ≠ description; `/categorias/carburo` leaf `Carburo`; `/categorias/llaves-herramientas-apriete` → `33 productos` (live count); `/categorias/lubricantes-multifuncionales` → `1 producto` + `Solicitar cotización`; `/categorias/extraccion-reparacion-fijaciones` → no `Filtrar subcategorías` | Validated | also asserted literally in `category-slug-metadata.test.ts`/`category-slug-error.test.tsx`'s 11 new rows |
| AC3 — entry points light up (16 hrefs, cards, sitemap, active state) | 2 | `/categorias` → 16 unique `href="/categorias/…"`; `/sitemap.xml` → 16 static `<loc>`; header trigger button on `/categorias/sujecion` carries the active `border-b-2` class (`isCategories` route match) | Validated (partial via curl) | Per-item `Sujeción (actual)` text lives inside HeroUI's `Dropdown.Popover`/mobile `Accordion`, which don't render into the initial SSR HTML `curl` fetches — 0 matches is expected, not a regression, already documented in the plan's own Manual section. Confirmed the trigger-level active state instead. Needs the manual click-through below to fully confirm the per-item highlight. |
| AC4 — static folders gone, `[slug]` error copy per category | 1, 2 | `ls src/app/categorias` shows only `[slug]`, `page.tsx`, `error.tsx`, `loading.tsx`; error copy | Cannot validate | error boundary needs a Strapi failure; proven instead by `category-slug-error.test.tsx`'s 16 rows (all 16 categories + the unknown-slug fallback) |
| AC5 — lint/tsc/build/test pass, table-driven tests, map-size counts | 1, 2 | n/a (automated commands) | Validated | `pnpm lint`, `pnpm exec tsc --noEmit`, `pnpm test` (44 suites, 453 passed, 1 pre-existing skip), `pnpm build` all pass |
