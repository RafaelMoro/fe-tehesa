...
- `pageFeedback` was added to `Home.tsx` because page-level catalog feedback belongs where the page mutation happens: category/brand requests, product replacement, local filter reset, and catalog pagination are all Home-owned. `useCatalogSearch.ts` stays focused on drawer/name-search validation and messages; moving category/brand page errors into that hook would couple it to Home's product-list and pagination ownership. `pageFeedback` prevents silent failures and stale page messages while keeping the hook smaller.
