...
|---|---|---|---|---|
| AC1 — Two-step on mobile only | 2 | `GET /` 200 + `GET /cotizar` 200, no hydration warnings | Cannot validate | The drawer mounts only after a client-side open, so no route response contains it. Proven by the Jest mobile block plus the 15 unmodified desktop/upgrade tests. Dev-server pass confirmed `/`, `/cotizar`, `/categorias/tornilleria-fijacion`, `/marcas`, `/marcas/weston` all 200 with the compiled importer. Server log was not tailable (pre-existing dev server, not started this session). **Manual 320/360/390 + ≥768px pass still owed by the user** — not run this session (no browser available). |
| AC2 — Step 1 selects, step 2 quantifies | 2 | — | Cannot validate | Interaction + responsive layout. Proven by Jest (toggle, `← Cambiar medidas`, no stepper in step 1, only selected medidas listed in step 2). **The two-column-at-320px OQ X check is unverified** — needs the manual pass. |
| AC3 — Footer reflects the step, in D11's words | 1, 2 | `GET /` 200, HTML contains `medidas` in the `ProductCard` CTA/counter and no `variante` in the body | Validated | Desktop-copy half curl-verified at `/` (Phase 1). Drawer footer strings (`Ninguna medida seleccionada` / `1 medida elegida` / `N medidas elegidas` / `1 medida · N pieza(s)` / `N medidas · M piezas`, CTA text) and the `CartVariantLine[]` equality on add are Jest-proven in the mobile block. |
| AC4 — Decrementing to 0 drops the medida | 2 | — | Validated | Jest: decrementing the sole selected medida to 0 removes it and returns to step 1 with `Continuar a cantidades` disabled. `NaN`-guard behavior is unchanged (still enforced by `QuantityStepper`, untouched). Colour-neutrality of the disabled CTA and the 320px manual pass are not Jest-checkable — owed by the user. |
| AC5 — Upgrade mode untouched; non-happy states follow Brief A | 3 | `GET /api/catalog/variants?documentId=<real>` 200 `{ success: true, data }`, no `CAT_ERR_*`; `GET /api/catalog/variants` returns the validation envelope; `GET /cotizar` 200 | Validated | Dev-server: `GET /api/catalog/variants?documentId=dwnvp7tdfbci2wn2yexfi090` → 200 `{"success":true,"data":[...]}`, no `CAT_ERR_*`; `GET /api/catalog/variants` (no `documentId`) → 400 `{"success":false,"code":"CAT_VAL_005","message":"Invalid documentId"}`; `GET /cotizar` → 200. The three rendered bodies (six-card skeleton with `role="status"`, neutral error with `role="alert"` + working `Reintentar` round-trip, rail-less empty state with bordered `Cerrar`) are Jest-proven; upgrade mode is held by its 3 unmodified tests. Exact skeleton/error colour tokens and the no-height-jump requirement are visual-only and remain owed by the user's manual pass. |
