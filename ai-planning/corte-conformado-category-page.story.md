...
| AC5 - Verification (lint, tsc, build, test, new/updated tests) | Phase 2 | Post-`pnpm build` re-run of `GET /categorias/herramientas-corte-conformado` 200 | Not validated | `pnpm exec tsc --noEmit`, `pnpm lint`, `pnpm test` pass after Phase 1; `pnpm build` and new Phase 2 tests still pending |
