...
| --- | --- | --- | --- | --- |
| AC1 – Abrasivos route + data | Phase 3 | `GET /categorias/abrasivos` 200; `<title>`, description meta, canonical, robots `index, follow`, JSON-LD `"name":"Abrasivos"`, `No hay productos disponibles.` alongside `<h1>` and `aria-label="Ruta"` | Not validated | Fetch path is the existing server action; `pnpm build` proves no new query |
| AC2 – Abrasivos page structure | Phase 3 (dropdown gating from Phase 1) | Same response: `aria-current="page">Abrasivos<`, `>Categoría<`, `<h1 …>Abrasivos industriales</h1>`, intro text, `Cotizar ahora`, `0 productos`, `Filtrar marcas`, **no** `Filtrar subcategorías`; `pnpm build` output lists `/categorias/abrasivos` with its own boundaries | Not validated | `Limpiar filtros` only appears after typing — covered by `__tests__/category-page` |
| AC3 – Shared UI, not a copy | Phase 1 | `GET /categorias/tornilleria` (Phase 1) / `/categorias/tornilleria-fijacion` (after Phase 2) 200 with the same H1/intro/placeholder/breadcrumb strings as before; `grep -c "Tornillería" src/features/CategoryPage/CategoryPage.tsx` is 0 | Not validated | "Byte-for-byte" is proven by the unchanged `CategoryPage.test.tsx` assertions + curl of the same strings |
| AC4 – Entry points | Phase 3 | `GET /categorias` contains `href="/categorias/abrasivos"`; `GET /sitemap.xml` contains `/categorias/abrasivos`; `git diff --stat` shows no change in `Header.tsx`, `MobileMenu.tsx`, `CategoryCard.tsx`, `sitemap.ts` | Not validated | Header dropdown active state for Abrasivos: manual check (rows don't SSR); mechanism covered by `Header.test.tsx` |
| AC5 – Tornillería rename | Phase 2 | `GET /categorias/tornilleria-fijacion` 200 with canonical + JSON-LD item on the new path; `GET /categorias/tornilleria` 404; `GET /sitemap.xml` has the new path and not the old; `GET /categorias` CTA href is the new path | Not validated | |
| AC6 – Verification | Phases 1–3 | `pnpm lint && pnpm exec tsc --noEmit && pnpm build && pnpm test` exit 0; new test files exist for abrasivos metadata/error, sitemap asserts both paths, `CategoryPage.test.tsx` has the "hides the subcategory dropdown" case | Not validated | Command-based, not a route check |
