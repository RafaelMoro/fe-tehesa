...
| AC5 - Dark mode + skeleton | Phase 1, 3 | `GET /` 200 contains `dark:bg-gray-900`, `dark:text-primary-100`, `dark:text-[#EDEDED]`; `pnpm design:lint`, `pnpm lint`, `pnpm exec tsc --noEmit`, `pnpm build`, `pnpm test` all pass | Cannot validate | `pnpm lint`, `tsc --noEmit`, `pnpm build` pass in Phase 1; `pnpm test` full suite and `design:lint` are gated to Phase 3. Visual match to "Tema oscuro" and skeleton no-shift are manual |
