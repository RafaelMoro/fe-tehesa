...
| --- | --- | --- | --- | --- |
| AC1 - Anatomy | Phase 1, 2 | `GET /` 200, contains `<article`, ` variantes</span>`, `Desde`, `hasta $`, `MXN</span>`, `<h2`; no `<img`, no `Hasta`, no `Foto del producto`. Null-guard cases proven by Phase 2 tests. | Cannot validate | Phase 1 dev-server curl confirmed all markers present/absent (incl. category kicker on `?mode=category&category=Carburo`); null-guard cases need Phase 2 tests. Order/type scale/colors are manual (browser at 1440px) |
...
| AC4 - Image slot | Phase 1, 2 | `GET /` 200 with **no** `<img` and no `aspect-[4/3]`; image-present case proven by the Phase 2 `getByRole("img", { name })` test | Cannot validate | Phase 1 curl confirmed no `<img`/`aspect-[4/3]` on `/`; image-present case needs Phase 2 test |
