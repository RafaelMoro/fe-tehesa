...
| --- | --- | --- | --- | --- |
| AC1 - Anatomy | Phase 1, 2 | `GET /` 200, contains `<article`, ` variantes</span>`, `Desde`, `hasta $`, `MXN</span>`, `<h2`; no `<img`, no `Hasta`, no `Foto del producto`. Null-guard cases proven by Phase 2 tests. | Cannot validate | Phase 1 dev-server curl confirmed all markers; Phase 2 tests confirm null-category/null-brand/null-price guards, pill `0`/`1`/`3` text (`getAllByText`, both breakpoint nodes). Order/type scale/colors are manual (browser at 1440px) |
| AC2 - Behavior parity | Phase 1, 2 | `GET /` 200 contains `Explorar las ` and `Agregar y elegir después`; the four untouched interaction tests pass in `pnpm test -- __tests__/product-listing/ProductCard.test.tsx` | Validated | Phase 1 curl confirmed both markers; 7/8 tests pass (only the pre-Phase-2 `Hasta` test fails as expected) including all four untouched interaction tests |
| AC3 - Responsive | Phase 1 | `GET /` 200 contains `lg:grid-cols-[repeat(auto-fill,minmax(280px,1fr))]` and `sm:grid-cols-2`; `grep` shows `loading.tsx` and `ProductListing.tsx` share the string | Cannot validate | Grid classes confirmed present via curl and grep (identical string in both files). Breakpoint layout (pill position, `hasta` placement, 44/40px, 4 columns at 1400px) is manual at ~390 / ~700 / ~1440px |
| AC4 - Image slot | Phase 1, 2 | `GET /` 200 with **no** `<img` and no `aspect-[4/3]`; image-present case proven by the Phase 2 `getByRole("img", { name })` test | Validated | Phase 1 curl confirmed no `<img`/`aspect-[4/3]` on `/`; Phase 2 added both the absent- and present-image tests, both pass |
| AC5 - Dark mode + skeleton | Phase 1, 3 | `GET /` 200 contains `dark:bg-gray-900`, `dark:text-primary-100`, `dark:text-[#EDEDED]`; `pnpm design:lint`, `pnpm lint`, `pnpm exec tsc --noEmit`, `pnpm build`, `pnpm test` all pass | Cannot validate | `pnpm design:lint`, `pnpm lint`, `tsc --noEmit`, `pnpm build`, `pnpm test` (383 passed/1 pre-existing skip) all pass; SSR HTML contains the D6 dark-token classes on the card. The old `text-emerald-700`/`dark:text-emerald-400` classes are still present page-wide but only from `CatalogHero.tsx` (out of scope, confirmed absent inside every `<article>` card). Visual match to "Tema oscuro" and skeleton no-shift are manual |
