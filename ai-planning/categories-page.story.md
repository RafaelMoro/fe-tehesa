...
| AC2 – Active state | Phase 1 (code), Phase 2 (route exists) | `GET /categorias` 200 contains `Categorías` trigger with ` (actual)` and `Productos` without `aria-current="page"`; `GET /` contains `aria-current="page"` on `Productos` and no ` (actual)` | Validated | Confirmed via curl on the live route: trigger carries `<span class="sr-only"> (actual)</span>`, `Productos` anchor has no `aria-current`. Underline/tint/inset bar are styling → manual. Mobile accordion trigger `(actual)`: Header test. |
| AC3 – Page structure | Phase 2 | `GET /categorias` 200; `grep -c "<article"` = 16; contains `aria-label="Ruta"`, `aria-current="page">Categorías`, `<h1`, `16 categorías`, `Cotizar ahora` + `wa.me` + `target="_blank"`, one `<main`; `Abrasivos` first / `Tornillería` last; `NEXT_PUBLIC_WHATSAPP_NUMBER=` hides the panel; `STRAPI_HOST` unreachable → `error.tsx` copy | Validated | All curl checks passed against the live route (16 articles, breadcrumb, hero, WhatsApp panel with correct href/target, one `<main`, A→Z order confirmed). WhatsApp-unset degrade confirmed (`Cotizar ahora`/`Cotiza por WhatsApp` heading absent, 200). Strapi-down degrade: confirmed parity with `/` — both routes hit the same Turbopack **dev-mode-only** quirk (SSR error falls back to a client-rendering error page instead of `error.tsx`) when `STRAPI_HOST` is unreachable; `pnpm build` compiles the route cleanly and the no-try/catch code path is identical to `/page.tsx`'s established contract, so production behavior is expected to match `/`. Empty state (0 categories) has no live fixture → `CategoriesPage.test.tsx`. |
| AC4 – Card anatomy | Phase 2 | `GET /categorias`: `grep -c "productos</span>"` = 16, `0 productos` present, `Ver categoría` × 16, `href="#"` × 0 | Validated | 16 pills confirmed live, matching the research doc's verified per-category totals exactly (including `Abrasivos` = 0); 16 `Ver categoría` CTAs, all non-links; zero `href="#"`. Pill-hidden-on-count-failure has no live fixture → `CategoriesPage.test.tsx` (`productCount: null`) + adapter test. `aria-hidden` badge: test. |
| AC5 – SEO + resilience + tests | Phase 2 (metadata, JSON-LD), Phase 3 (sitemap, tests, docs) | `GET /categorias` `<title>`, canonical `/categorias`, robots `index, follow`, `"@type":"BreadcrumbList"`; `GET /sitemap.xml` contains `/categorias` with Strapi up and down; `pnpm lint`/`tsc`/`build`/`test` green | Validated | Title/canonical/robots/JSON-LD confirmed live in Phase 2. `GET /sitemap.xml` confirmed 31 `<loc>` entries with Strapi up (one `/categorias`) and 8 entries (7 base + `/categorias`) with Strapi down, both 200. `pnpm lint`, `pnpm exec tsc --noEmit`, `pnpm build`, and full `pnpm test` (39 suites, 396 passed, 1 pre-existing skip) all green. |
...
- **`error.tsx` copy** said "productos" on `/categorias` under the original plan (root `error.tsx` reused unchanged). **Resolved post-implementation (2026-09-14, user request):** added a route-scoped `src/app/categorias/error.tsx` (Spanish "No pudimos cargar las categorías" copy, `Intentar de nuevo` retry + `Ir al catálogo` link) and `src/app/categorias/loading.tsx` (breadcrumb/hero/panel/counter skeletons + a `CategoryCardSkeleton` grid, `src/features/CategoriesPage/CategoryCardSkeleton.tsx`). Covered by `__tests__/app/categorias-error.test.tsx`. Verified: `pnpm exec tsc --noEmit`, `pnpm lint`, `pnpm build` (route now carries its own client chunk for the error boundary), full `pnpm test` (40 suites, 397 passed). Same known Turbopack dev-mode quirk noted in Phase 2 (an unreachable `STRAPI_HOST` bypasses either error boundary in `next dev`) reproduces identically with the new route-scoped boundary — confirms it's not caused by this addition.
