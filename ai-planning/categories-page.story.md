...
| AC5 – SEO + resilience + tests | Phase 2 (metadata, JSON-LD), Phase 3 (sitemap, tests, docs) | `GET /categorias` `<title>`, canonical `/categorias`, robots `index, follow`, `"@type":"BreadcrumbList"`; `GET /sitemap.xml` contains `/categorias` with Strapi up and down; `pnpm lint`/`tsc`/`build`/`test` green | Not validated | Title/canonical/robots confirmed live in Phase 2; JSON-LD script confirmed present (verified via targeted grep on the raw response, not the `"@type":"..."` count check as written, which undercounts due to JSON string-escaping in the flight payload). Sitemap entry and full `pnpm test` land in Phase 3. |
