...
|---|---|---|---|---|
| AC1 – Desktop nav | Phase 1 | `GET /` 200 contains `Productos` with `aria-current="page"`, `Categorías`, `Marcas`; `GET /cotizar` `Productos` without `aria-current` | Not validated | Dropdown items/active highlight/close paths are in a closed popover (not in SSR HTML): proven by `__tests__/shared/Header.test.tsx` + manual keyboard path |
| AC2 – Mobile side menu | Phase 2 | `GET /` 200 contains `aria-label="Menú"` + `aria-expanded="false"` | Not validated | Drawer contents, focus return, 44px targets, one-open accordion: jest + manual at 390px |
| AC3 – Lupa opens existing drawer | Phase 2 | `GET /` contains `aria-label="Buscar"`; `GET /cotizar` does not | Not validated | Open path: `Home.test.tsx` event test + manual tap |
| AC4 – Utility bar → WhatsApp | Phase 3 | `GET /` 200 contains copy A + `wa.me/<number>?text=…` anchor with `target="_blank" rel="noopener noreferrer"`; with `NEXT_PUBLIC_WHATSAPP_NUMBER` empty, tagline present and no `wa.me` | Not validated | |
| AC5 – Resilience + theme | Phase 1 (sticky, degrade, every route), Phase 3 (tests/docs) | `GET /cotizar` 200 with `STRAPI_HOST` unreachable and no `Categorías`; `pnpm build` passes listing `/` and `/cotizar` as dynamic | Not validated | Sticky + dark tokens are styling → manual; tests/docs → `pnpm test` green |
