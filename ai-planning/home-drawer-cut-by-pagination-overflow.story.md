...
| AC2 - drawer exactly viewport width on `/` below `md`, `md`/`lg` widths kept | Phase 1, 2 | Chrome dialog rect `left 0`/`right clientWidth` | Cannot validate | `isFullWidthOnMobile` prop added and wired on home's call site only (`max-md:w-full! max-md:max-w-none!` beats both HeroUI defaults); `ProductVariantsDrawer.test.tsx` green. Drawer is client-rendered after click and Claude in Chrome is unavailable in this session, so the dialog rect (`left 0`/`right clientWidth`) still needs a Chrome pass |
...
| AC5 - other routes and filtered mode unchanged, default drawer width | Phase 2, 3 | `GET /categorias/<slug>`, `/marcas`, `/cotizar`, `/?mode=category…` all 200, filtered pagination markup unchanged | Cannot validate | `CategoryPage`, `BrandPage`, `QuotePage` call sites unedited, confirmed via grep — they don't pass `isFullWidthOnMobile`, so default `false` keeps today's width. `GET /categorias/extraccion-reparacion-fijaciones`, `/marcas`, `/cotizar` all 200, no server errors. Actual rendered width on those routes still needs a Chrome pass (unavailable this session) |
