...
| --- | --- | --- | --- | --- |
| AC1 - Hero copy, count label, panel, drawer button, loading/error compat | Phase 1 | `GET /` 200 contains kicker/H1/intro/`333 productos en catálogo`/panel title+body+footer/`Buscar en todo el catálogo`; `GET /?mode=brand&brand=Weston&page=1` contains `50 productos` | Validated | Confirmed via curl on `pnpm dev`; drawer opening (`Home - hero`) and loading/error labels (`error.test.tsx`) also green |
| AC2 - Brand strip | Phase 2 | `GET /` 200 contains `aria-label="Marcas en almacén"`, six `/marcas/<slug>` hrefs in `BRAND_PAGES` order, `Cleveland` not `Clevaland`, `href="/marcas"` | Validated | Confirmed via curl; `Clevaland`/`Marca Libre` confirmed absent from the `<nav>` scope specifically. Empty-taxonomy case covered by Jest only (Strapi failure never reaches `Home` live) |
| AC3 - Filter row + hint + popover copy | Phase 2 | `GET /` 200 contains the new `placeholder` and `Escribe el nombre del producto. Ejemplo:` | Validated | Confirmed via curl; hint hide/show and popover text covered by Jest `Home - filter hint` |
| AC4 - No-result paragraphs | Phase 3 | — | Cannot validate | Local filtering is client-only; proven by Jest `Home - no results` (3 cases: actions present, drawer opens, clear restores grid+hint, filter-only message). Category/brand panel parity unchanged (`ProductListing.tsx` untouched) |
| AC5 - Pagination labels, style, end copy | Phase 3 | `GET /` (no end copy, next is a link), `GET /?page=7` (end copy, next disabled), `GET /?mode=brand&brand=Clevaland&page=1` (end copy, 5 products/no next), `GET /?mode=brand&brand=Weston&page=1`/`page=2` (page 1: no end copy, page-2 href; page 2: end copy, no next) | Validated | Confirmed via curl against live Strapi data. Outlined visual style is manual. Live Weston brand count now exceeds the research's assumed 90 (page 3 returns products rather than the redirect-to-notice=end case), so that specific redirect edge wasn't exercised live — it is unrelated to this story's changes (`page.tsx` redirect logic is untouched) |
| AC6 - Closing panel | Phase 1 | `GET /` 200 contains `href="/cotizar"`, `Ver mi lista de cotización`, `wa.me`, `target="_blank"`, `rel="noopener noreferrer"` | Validated | Confirmed via curl with `NEXT_PUBLIC_WHATSAPP_NUMBER` set. Unset-number case covered by Jest (`hides the WhatsApp link when no number is set`) rather than a second dev-server restart |
| AC7 - Tests | Phases 1–3 | `pnpm test -- __tests__/home` green, then full `pnpm test` | Validated | `__tests__/home/Home.test.tsx`: 30 passed, 1 pre-existing skip. Full `pnpm test`: 513 passed, 1 pre-existing skip, 49 suites. `pnpm build` also green |
